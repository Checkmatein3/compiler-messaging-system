...
class postEntry {
    ...
    int submitPost(struct userPost* postToSubmit) {
        char* streamUsersFileName = createStreamUsersFileName(postToSubmit->streamname);

        /*check that the file exists*/
        int streamExistence = access(streamUsersFileName, F_OK);
        if (streamExistence == -1) {
            printf("The '%s' stream does not exist.\n", postToSubmit->streamname);
            free(streamUsersFileName);
            return 1;
        }

        /*check that the user has permission to post to the stream*/
        if (!containsUser(streamUsersFileName, postToSubmit->username)) {
